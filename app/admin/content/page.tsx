...
const ContentEditor = () => {
  ...
  const fetchContent = async () => {
    const { data, error } = await supabase.from("project_content").select("*");
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">
          Edit your project&apos;s content and descriptions
        </p>
      </div>
      ...
      <Card className="bg-gradient-card border-border">
        ...
        <CardContent className="space-y-4">
          ...
          <Button
            onClick={() =>
              handleUpdate("about_description", content.about_description)
            }
      ...
      <Card className="bg-gradient-card border-border">
        ...
        <CardContent className="space-y-4">
          ...
          <Button
            onClick={() =>
              handleUpdate("vision_statement", content.vision_statement)
            }
            className="bg-gradient-primary hover:shadow-glow-purple transition-all"
          >
            <Save className="w-4 h-4 mr-2" />
